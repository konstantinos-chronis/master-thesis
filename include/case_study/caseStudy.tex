...
\lettrine[lines=4, loversize=-0.1, lraise=0.1]{T}{his} chapter aims to check whether the tools claiming to measure agility, actually do. It presents the case study performed in company A, along with the research questions and research methodology followed.
...
The last years Laurie Williams conducted two case studies \cite{Williams_Microsoft, laurie_williams} concerning the agile practices which are highly thought by practitioners. We picked the top of them in order see at what extent the tools cover practices essential for software development in the industry. For identifying which practices from the case studies apply to the three tools we used the analysis made in Appendix~\ref{ch:mapping}. In there, all questions were separated based on the agile practice in which they belonged.
