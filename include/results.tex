...
The groups of direct match questions showed some unexpectedly amazing results. One would expect that questions which are considered the same would yield the same results. On the contrary, this did not happen for any of the groups, apart from the questions concerning the \textit{Software Control Management} group. The heatmaps which were generated by the answers made it very clear that the respondents gave different scores. 

\begin{table} [H]
	\begin{tabular}{| p{1cm} | p{2cm} | p{1cm} | p{2cm} | p{1cm} | p{2cm} |} \hline
		Group & Frequency & Group & Frequency & Group & Frequency \\ \hline
		\hyperref[G1]{G1} &f & \hyperref[G2]{G2} & f& \hyperref[G3]{G3} &f \\ \hline
		\hyperref[G4]{G4} &f & \hyperref[G5]{G5} & f& \hyperref[G6]{G6} &f \\ \hline
		\hyperref[G7]{G7} &f & \hyperref[G8]{G8} & f& \hyperref[G9]{G9} &f \\ \hline
		\hyperref[G10]{G10} &f & \hyperref[G11]{G11} & f& \hyperref[G12]{G12} &f \\ \hline
		\hyperref[G13]{G13} &f & \hyperref[G14]{G14} & f& \hyperref[G15]{G15} &f \\ \hline
		\hyperref[G16]{G16} &f & \hyperref[G17]{G17} & f& & \\ \hline
	\end{tabular}
	\caption{}
	\label{}
\end{table}

Explain which had most common questions and which didn't


The only group of questions in which all the respondents gave the exact same answer was about the existence of \textit{Software Control Management}. Company A uses version control management software for every line of code written.
...
As it was explained in section~\ref{sec:direct_match_results} almost all groups had different responses for the same questions. This could be due to two reasons. The first one, is that the groups of direct match questions were not correctly formed and and the second one, is that people have the tendency to judge differently a question. 
As far as the aspect of the groups of direct match questions not being correctly formed in concerned, it is considered to have a low possibility since they were verified by employees whose opinion was asked as it was mentioned in . \todo{add this in methodology and add the reference}.
On the other hand, according to \citet{Lacy} survey respondents tend to give different answers to the same questions even weeks apart, something which is a common issue in surveys.

%http://en.wikipedia.org/wiki/Social_desirability_bias
%http://en.wikibooks.org/wiki/Social_Research_Methods/Surveys
%http://www.uk.sagepub.com/upm-data/23854_Chapter2.pdf
%http://ecpr.eu/Filestore/PaperProposal/b0647350-d971-4a09-aefd-373c8ad1c4be.pdf
...
The only group of questions in which all the respondents gave the exact same answer was about the existence of \textit{Software Control Management}. Company A uses version control management software for every project.
