...
Coming to question \textbf{\textit{``Will the questions that are exactly the same among the tools yield the same results?"}} we saw that a respected amount of respondents' answers were different.

Coming to question \textbf{\textit{``What is the coverage of agile practices among tools?"}} we saw that the tools did not cover at the same level the agile practices, neither in terms of questions plethora nor to agile practices number.

Thus, we conclude that \ac{PAM}, \ac{TAA} and \ac{OPS} do not measure agility in the same way, although they should. The reasons for these unexpected phenomenon are explained in the next paragraphs.
